...
function removeLastOccurrences(str, value) {
  ...
  const str2 = str.slice(str.lastIndexOf(value));

  return str1.concat(str2.replace(value, ''));
}
...
function isPalindrome(str) {
  const strR = str.toLowerCase().split('').reverse().join('');
  const strRC = strR.replaceAll(/[/.,?!]/g, '').replaceAll(' ', '');
  const strI = str.toLowerCase();
  const strIC = strI.replaceAll(/[/.,?!]/g, '').replaceAll(' ', '');
...
function reverseWords(str) {
  ...
  for (let i = 0; i < words.length; i += 1) {
    wordReversed = words[i].split('').reverse().join('');
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i].match(/[A-Z]/)) {
      if (str[i] < 'N') {
        result = `${result}${String.fromCharCode(str[i].charCodeAt() + 13)}`;
      } else {
        result = `${result}${String.fromCharCode(str[i].charCodeAt() - 13)}`;
      }
